...
public class SharedPreference {
    ...
    /**
     * Save a file path to SharedPreferences with the given key
     *
     * @param key  SharedPreferences key
     * @param path File path to be saved
     */
    public void putFilePath(String key, String path) {
        checkForNullKey(key);
        checkForNullValue(path);
        preferences.edit().putString(key, path).apply();
    }

    /**
     * Get a file path from SharedPreferences with the given key
     *
     * @param key SharedPreferences key
     * @return File path stored at the given key, or an empty string if not found
     */
    ...
    public void putFile(String key, File file) {
        checkForNullKey(key);
        checkForNullValue(file);

        String filePath = (file != null) ? file.getAbsolutePath() : null;
        putString(key, filePath);
    }
    public ArrayList<File> getListFile(String key) {
        ArrayList<String> filePaths = getListString(key);
    ...
    public void putListFile(String key, ArrayList<File> fileList) {
        checkForNullKey(key);
        checkForNullValue(fileList);
     ...
     * @param value the pref value to check
     */
    private void checkForNullValue(Object value) {
        if (value == null) {
            throw new NullPointerException();
        }
    }
